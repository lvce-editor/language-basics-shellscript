...
const RE_KEYWORD =
  /^(?:7za|alias|ar|awk|bg|bind|break|builtin|bunzip|bunzip2|caller|case|cargo|cat|catdog|cd|command|compgen|complete|continue|curl|dirs|disown|dpkg|do|done|echo|elif|else|enable|esac|eval|eval_gettext|exec|exit|false|fc|fg|fi|for|function|getopts|grep|gzip|hash|help|history|identity|if|in|jobs|kill|let|lha|logout|lunzip|lzip|lzma|mkdir|miniunz|miniunzip|npm|node|pdftotext|popd|printf|pushd|pwd|rar|read|readonly|rm|rpm|sed|set|shift|shopt|sleep|snap|source|suspend|return|tar|test|then|times|trap|true|type|ulimit|umask|unalias|unrar|unset|unwrapdiff|unzip|unzoo|xz|wait|while|zoo|zstd|umask)\b/
